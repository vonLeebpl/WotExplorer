...
cp /vagrant/.provision/helpers/ngxcb_symfony.sh ./ngxcb-symfony
sudo chmod guo+x ngxen ngxdis ngxcb ngxcb-symfony
sudo mv ngxen ngxdis ngxcb ngxcb-symfony /usr/local/bin

# Create Nginx Server Block named "vagrant" and enable it
sudo ngxcb-symfony -d $public_folder -s "$1.xip.io$hostname" -e -f
